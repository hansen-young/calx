...
def validate_arguments(func: Optional["AnyCallableT"], args: dict):
    """
    Decorator to validate the arguments passed to a function.
    Raise pydantic.ValidationError if failed
    """
...
def _is_pydantic_supported(type_: Type[Any]):
    ...
    g = find_validators(type_, config=DisableArbitraryTypes)
    try:
        next(g)
    except RuntimeError:
        return False
...
class ValidatedFunction:
    def __init__(self, function: "AnyCallableT", config: "ConfigType"):  # noqa C901
        ...
        real_parameters: Dict[str, Parameter] = {}

        # convert classes not supported by pydantic to pydantic's BaseModel
        ...
        for i, (name, p) in enumerate(real_parameters.items()):
            if p.annotation is p.empty:
                annotation = Any
            else:
                annotation = real_parameters[name].annotation
